...
def work(fnames):
    ...
    for iG in range(Group2_No):
        for t in range(timeStep):
            ...
            plotNOI, = pyplot.plot(range(featureNo), tmp[t,:], 'o-', color = color, label = 'AD', alpha = 0.7)
        iGroup2 += 1
    ...
    for iG in range(1):
        for t in range(1):
            ...
            tmp = tmp.reshape(timeStep, featureNo)
            color = ((iGroup1+Group1_No)/(Group1_No+Group1_No),0,0)
            plotORI, = pyplot.plot(range(timeStep), tmp[:,t], 'o-', color = color, label = 'AD', alpha = 0.7)
        iGroup1 += 1
    ...
    for iG in range(1):
        for t in range(1):
            ...
            tmp = tmp.reshape(timeStep, featureNo)
            color = (0, (iGroup2+(Group2_No))/(Group2_No+(Group2_No)), 0)
            plotNOI, = pyplot.plot(range(timeStep), tmp[:,t], 'o-', color = color, label = 'NC', alpha = 0.7)
        iGroup2 += 1
