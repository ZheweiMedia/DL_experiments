...
def label_for_keras(_label):
    output = numpy.empty((_label.shape[0], _label.shape[1],3))
    for iNo in range(_label.shape[0]):
        for fNo in range(_label.shape[1]):
            if _label[iNo, fNo] == 0:
                output[iNo, fNo,:] = [1,0,0]
            if _label[iNo, fNo] == 1:
                output[iNo, fNo, :] = [0,1,0]
            if _label[iNo, fNo] == 2:
                output[iNo, fNo, :] = [0,0,1]
    return output
...
print (numpy.where(train_label == 2)[0].shape[0])
...
print ('Baseline of training is:',(numpy.where(train_label == 2)[0].shape[0])/train_label.shape[0]/train_label.shape[1])
print ('Baseline of training is:',(numpy.where(train_label == 3)[0].shape[0])/train_label.shape[0]/train_label.shape[1])
print ('Baseline of validation is:', (numpy.where(valid_label == 2)[0].shape[0])/valid_label.shape[0]/valid_label.shape[1])
print ('Baseline of validation is:', (numpy.where(valid_label == 3)[0].shape[0])/valid_label.shape[0]/valid_label.shape[1])
