...
trainPair = data_generator(trainlinks, trainannotlinks, config, config["batch_size"])
validPair = data_generator(testlinks, testannotlinks, config, config["batch_size"], rdm=False)

train_generator = trainPair
validation_generator = validPair
nb_train_samples = 640 #640 1460
nb_test_samples = 380 #260 760



"""
model = unet_model_3d(config, input_shape=config["input_shape"],
...
"""
# test the loss result
model = load_old_model(config["model_file"])
evaluate_list = list()
total_eval = 0
total_No = 0
for sample, target in train_generator:
    if total_No < 14600:
...
for sample, target in validation_generator:   
    for sampleNo in range(sample.shape[0]):
        if (predID < 7613):
            ...
            each_loss = model.evaluate(each_sample, each_target, verbose = 0)[1]
